...
router
  ...
  .get(verifyAccessToken, async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ err: 'An error occurred while searching for notes' });
    }
  })
  .post(verifyAccessToken, async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ err: 'An error occurred while creating note' });
    }
  });
...
router
    ...
    try {
      ...
      if (note) {
        if (note.authorId.toString() !== res.locals.user.id) {
          return res
            .status(401)
            .json({ err: 'Unauthorized to view this note' });
        }
      }
      ...
      return res
        .status(500)
        .json({ err: 'An error occurred while searching for note' });
    }
  })
  .patch(verifyAccessToken, async (req, res) => {
    ...
    if (!req.body.title && !req.body.body) {
      return res
        .status(422)
        .json({ err: 'Either a title or a body needs to be provided' });
    }
    ...
    try {
      ...
      if (
        (req.body.title === note.title && !req.body.body) ||
        (req.body.body === note.body && !req.body.title) ||
        (req.body.body === note.body && req.body.title === note.title)
      ) {
        return res
          .status(422)
          .json({ err: 'No changes from original note were provided' });
      }
      ...
      return res
        .status(500)
        .json({ err: 'An error occurred while updating note' });
    }
  })
  .delete(verifyAccessToken, async (req, res) => {
    try {
      ...
      if (note.authorId.toString() !== res.locals.user.id) {
        return res
          .status(401)
          .json({ err: 'Unauthorized to delete this note' });
      }
      ...
      return res
        .status(500)
        .json({ err: 'An error occurred while deleting note' });
    }
  });
